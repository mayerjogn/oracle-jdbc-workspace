/*
    ...
    - 관계형 데이터베이스는 최소한의 데이터로 각각의 테이블에 담고 있음
     ...
     부서 데이터는 부서 테이블, 사원에 대한 데이터는 사원 테이블, 직급 테이블 등등...
     ...
     - 크게 "오라클 구문"과 "ANSI 구문"이 두가지 있음
     - ANSI : 미국국립표준협회 == 산업표준을 제정하는 단체에서 만든 표현
...
/*
    1. 오라클 - 등가 조인(EQUAL JOIN) / ANSI - 내부 조인(INNER JOIN / NATURAL JOIN)
        - 연결시키는 컬럼의 값이 일치하는 행들만 조인돼서 조회 (일치하는 값이 없는 행은 조회가 안됨)
        ...
        1) 오라클 전용 구문
           ...
           - FROM 절에 조회하고자 하는 테이블들을 콤마로(,) 구분하여 나열함
        ...
        2) ANSI 표준 구문
           ...
           - JOIN 절에서 같이 조회하고자 하는 테이블을 기술 후 매칭 시킬 컬럼에 대한 조건을 기술함 (USING 구문과 ON 구문 사용) 
           - 연결에 사용하려는 컬럼명이 같은 경우 ON 구문 대신에 USING(컬럼명) 구문사용
...
-- 해결방법2) 테이블에 별칭을 부여해서 이용하는 방법 1방법은 테이블명이 너무 길때 사용하기 힘듬

SELECT EMP_ID, EMP_NAME, J.JOB_CODE, JOB_NAME
FROM EMPLOYEE E, JOB J
WHERE E.JOB_CODE = J.JOB_CODE;

-- ANSI 구문 컬럼명이 다른경우라 ON 사용
...
FROM EMPLOYEE E, JOB J
WHERE E.JOB_CODE = J.JOB_CODE AND JOB_NAME = '대리'; -- JOIN 후 AND를 사용해야함
...
SELECT EMP_ID, EMP_NAME,BONUS,0, DEPT_TITLE
FROM EMPLOYEE , DEPARTMENT 
    WHERE DEPT_CODE = DEPT_ID 
