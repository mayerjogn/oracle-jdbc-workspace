...
/*
    2. 오라클 - 포괄 조인 / ANSI - 외부 조인 (OUTER JOIN)
        - 두 테이블간의 JOIN 시 일치하지 않는 행도 포함시켜서 조회가 가능
        - 단, 반드시 기준이 되는 테이블(컬럼)을 지정해야 한다.(LEFT, RIGHT, FULL, (+))등이 사용될것         
*/
-- 사원명, 부서명, 급여, 연봉 조회
...
SELECT EMP_NAME,DEPT_TITLE,SALARY,SALARY*12
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+); -- 기준으로 삼고자하는 테이블의 반대편 테이블의 컬럼 뒤에 (+)를 붙인다.
...
-->> 오라클 구문 -- FULL JOIN은 오라클 구문으로는 사용못함
--SELECT EMP_NAME,DEPT_TITLE,SALARY,SALARY*12
--FROM EMPLOYEE, DEPARTMENT
--WHERE DEPT_CODE = DEPT_ID;
...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB ON(DEPT_TITLE = JOB_NAME)
JOIN NATIONAL ON (NATIONAL_CODE = LOCAL_NAME)
JOIN SAL_GRADE ON(SALARY BETWEEN MIN_SAL AND SAL_LEVEL);
...
-- 3. 보너스를 받는 직원들의 직원명, 보너스, 연봉, 부서명, 근무지역 조회 단, 부서 코드가 없는 사원도 출력할 수 있게//

--ANSI//
SELECT EMP_NAME , BONUS, SALARY*12, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON(E.DEPT_CODE = D.DEPT_ID)
LEFT JOIN LOCATION L ON(D.LOCATION_ID = L.LOACL_CODE)
WHERE BONUS IS NOT NULL;
...
WHERE E.DEPT_CODE = D.DEPT_ID
AND D.LOCATION_ID = L.LOACL_CODE
AND BONUS IS NOT NULL;
...
-- 5. 각 부서별 평균 급여를 조회하여 부서명, 평균 급여(정수 처리)를 조회 HAVING절 AVG 사용 
...
--급여 등급이 S1, S2인 경우 '고급' 급여 등급이 S3, S4인 경우 '중급' 급여 등급이 S5, S6인 경우 '초급'
