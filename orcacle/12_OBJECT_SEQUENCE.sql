...
/*
    ...
    [MAXVALUE 숫자]            --> 최대값 지정 (기본값 존나큼)
    
    [MINVALUE 숫자]            --> 최소값 지정 (기본값 1)   
    
    [CYCLE | NOCYLCE]         --> 값 순환 여부 지정 (기본값 NOCYCLE)
...
/*
    ...
    시퀀스명.NEXTVAL : 시퀀스값에 일정값을 증가시켜서 발생된 값
                     현재 시퀀스 값에서 INCREMNET BY 값 만큼 증가된 값
                     시퀀스명.CURRVAL + INCREMENT BY 값
*/

-- NEXTVAL을 한 번이라도 수행하지 않은 이상 CURRVAL을 가져올 수 없음
...
SELECT SEQ_EMPID.NEXTVAL FROM DUAL; -- 맥스값 초과로 에러 발생
...
/*
    ...
    [CYLCE | NOCYLCE]--> 값 순환 여부 지정(기본값 NOCYCLE)
    
    [NOCACHE | CACHE 바이트크기] --> 캐시메모리 할당 (기본값 CACHE 0)
...
-- 2) 매번 새로운 발생되는 SEQUENCE 사용해서 INSERT 구문 작성
