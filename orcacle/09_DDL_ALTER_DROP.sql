...
ALTER TABLE DEPT_COPY MODIFY LNAME DEFAULT '미국'; -- 당장 있던 자료들 기본값이 미국으로 바뀌는게아니라 추가된 자료들 기본값이 미국으로바뀜
...
ALTER TABLE DEPT_COPY DROP COLUMN LNAME; -- 오류 테이블에 최소 한개의 컬럼은 있어야하니

ROLLBACK; -- 컬럼 삭제는 롤백안됨

ALTER TABLE MEMBER_GRADE DROP COLUMN GRADE_CODE; -- 부모 키 열 삭제 불가능
--> 참조되고 있는 컬럼이 있다면 삭제 불가능
...
ALTER TABLE DEPT_COPY RENAME COLUMN LNAME TO LCOATION_NAME;
...
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_COPY_DEPT_ID_PK PRIMARY KEY(DEPT_ID); --이런 식으로
...
-- 만약 삭제하고자 한다면
-- 방법 1. 자식테이블을 먼저 삭제한 후 부모테이블 삭제하는 방법 서순 중요
DROP TABLE MEMBER;
