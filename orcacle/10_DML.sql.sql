/*
    DML(Data Manipulation Language) : 데이터 조작 언어로 테이블에 값을 삽입(INSERT)하거나,
    ...
    2) INSERT INTO 테이블명(컬럼명, 컬럼명, ...., 컬럼명)
                                                 VALUES(값, 값, ..., 값);
        - 테이블의 특정 컬럼에 대한 값만 INSERT 하고자 할때
        - 선택 안된 컬럼들은 기본적으로 NULL 값이 들어감 NOT NULL 조건이 있는경우 무조건 값을 넣어야함
        - 기본값(DEFAULT)이 지정되어 있으면 기본값이 들어감
        
    3) INSERT INTO 테이블명 (서브쿼리);
        - VALUES 대신 서브쿼리로 조회한 결과값을 통째로 INSERT 한다
        - 서브쿼리의 결과값이 INSERT 문에 지정된 테이블 컬럼의 개수와 데이터 타입이 같아야 한다
*/  
DROP TABLE EMP_01;
...
-- 컬럼순서와  데이터 타입이 맞지않아서 에러발생 '유아지원팀',300
INSERT INTO EMP_01
VALUES('유아지원팀', 300, '위하준', DEFAULT);


-- 표현법 2) 컬럼명 명시해서 넣으면 들어감
...
VALUES(700, '마케팅팀'); -- NAME NOT NULL 선언해놔서
...
/*
INSERT INTO EMP_01 ( 강사님꺼
    SELECT EMP_ID, EMP_NAME, DEPT_TITLE, HIRE_DATE
    FROM EMPLOYEE
    JOIN DEPARTMENT ON (DEPT_CODE = DEPTID)
);
...
-- EMP_MANAGER 테이블에 EMPLOYEE 테이블의 부서 코드가 D1인 직원의 사번
-- 이름, 관리자 사번을 조회하여 삽입한다.
...
-- EMPLOYEE 테이블의 입사일 기준으로 2000년 1월 1일 이전에
...
/*
    ...
    - UPDATE시 서브 쿼리를 사용해서 서브 쿼리를 수행한 결과값으로 컬럼 값을 변경할 수 있다
*/
...
-- EMP_SALARY, DEPT_COPY, LOCATION JOIN
SELECT EMP_ID
...
*/
    ...
    -- EMP_SALARY 테이블에서 DEPT_CODE가 D5인 직원들을삭제
