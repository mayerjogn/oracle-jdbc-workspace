/*
    ...
    >> SELECT 절에 단일행 함수와 그룹 함수는 함께 사용하지 못한다.
        왜? 결과 행의 갯수가 다르기 때문에
...
/*
    INSTR
    - INSTR(STRING, STR [, POSITION, OCCURRECNCE])
    ...
    - POSITION : 찾을 위치의 시작 값(기본값 1)<- 명시를 하지않으면 기본값 1
        1 : 앞에서부터 찾는다.
       -1 : 뒤에서부터 찾는다.
       
    - OCCURRENCE : 찾을 문자값이 반복될 때 지정하는 빈도(기본값 1)<- 명시를 하지않으면 기본값 1 
                    음수 사용 불가함
*/
...
SELECT EMAIL, INSTR(EMAIL,'@'), EMAIL, INSTR (EMAIL,'s', -1,2)
FROM EMPLOYEE
WHERE EMAIL LIKE '%s%';
...
/*
    ...
    - 문자열에 덧붙이고자하는 문자를 왼쪽 또는 오른쪽에 덧붙여서
      최종 N길이만큼의 문자열을 반환
*/
...
/*
    ...
    - 문자열의 왼쪽 또는 오른쪽에서 제거하고자하는 문자들을 찾아서 제가한 나머지 문자열을 반환한다
*/
...
/*
    ...
    - TRIM([LEADING | TRALING | BOTH] 제거하고자하는 문자들 FROM STRING)
*/
...
SELECT TRIM(BOTH 'z' FROM'ZZZKHZZZ')FROM DUAL;-- 양쪽으로 제거
...
/*
    ...
    - POSITION : 문자열을 추출한 시작위치값(음수값도 가능)
...
SELECT EMP_NAME
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 8,1) IN(1,3); --= '1';
...
SELECT CONCAT(CONCAT('가나다라','ABCD'), '1234')FROM DUAL; -- 여러개의 문자를 합칠때는 CONCAT을 밖에 한번더 써야함
...
*/
SELECT MOD(10,3) FROM DUAL; --10을 3으로 나눴을때 나머지값 반환
...
SELECT ROUND(123.465, -2)FROM DUAL;
...
*/
SELECT TRUNC(123.952)FROM DUAL;-- 위치 선정안할 시 0이라 생각해서 소수점 다버림
...
-- 직원명, 입사년도, 입사월, 입사일 조회
SELECT EMP_NAME, EXTRACT(YEAR FROM HIRE_DATE )"입사 년도", EXTRACT(MONTH FROM HIRE_DATE)"입사 월"
, EXTRACT(DAY FROM HIRE_DATE)"입사일"
FROM EMPLOYEE
/*ORDER BY EXTRACT(YEAR FROM HIRE_DATE ), 2.
EXTRACT(MONTH FROM HIRE_DATE) ASC, 3.
EXTRACT(DAY FROM HIRE_DATE)ASC; 4. */  -- 둘다가능
...
*/
SELECT TO_CHAR(1234, 'L999999')FROM DUAL; -- L 현재 설정된 나라의 화폐 단위
...
SELECT EMP_NAME"직원명", TO_CHAR(SALARY,'L99,999,999')"급여", TO_CHAR(SALARY*12,'L99,999,999')"연봉"
FROM EMPLOYEE
ORDER BY "연봉";

-- 날짜 -> 문자
SELECT TO_CHAR(SYSDATE,'PM HH:MI:SS')FROM DUAL; -- 1~12시
SELECT TO_CHAR(SYSDATE,'AM HH24:MI:SS')FROM DUAL; -- 0~24시
...
-- 년도와 관련된 포맷
SELECT TO_CHAR(SYSDATE,'YYYY'), TO_CHAR(SYSDATE,'YY'), TO_CHAR(SYSDATE,'RRRR'),
TO_CHAR(SYSDATE,'RR'),TO_CHAR(SYSDATE,'YEAR') -- 이건 문자열로 나옴
FROM DUAL;

-- 월과 관련된 포맷
SELECT TO_CHAR(SYSDATE,'MM'),TO_CHAR(SYSDATE,'MON'),TO_CHAR(SYSDATE,'MONTH')
,TO_CHAR(SYSDATE,'RM') -- 뭔 로마자여
From DUAL;

-- 일과 관련된 포맷    -- 주 기준             -- 월 기준                --년 기준
...
SELECT EMP_NAME "직원명", TO_CHAR(HIRE_DATE,'YYYY "년" MON DD "일" (DY)') "입사일"
FROM EMPLOYEE
ORDER BY "입사일";
...
SELECT TO_DATE('20230621 033823', 'YYYYMMDD HH24:MI:SS')FROM DUAL;
...
/*
    선택 함수 : 여러 가지 경우에 선택을 할 수 있는 기능을 제공하는 함수 switch랑 비슷
...
-- 사원명, 직급코드, 기존급여, 인상된 급여를 조회 
-- 직큽 코드가 J7인 사원은 급여를 10% 인상
-- 직급 코드가 J6인 사원은 급여를 15% 인상
-- 직급 코드가 J5인 사원은 급여를 20% 인상
-- 그 외의 직급의 사원은 급여를 5%만 인상
-- 정렬 : 직급 코드가 J1 부터, 인상된 급여 높은 순서대로 
SELECT EMP_NAME, JOB_CODE, SALARY, DECODE(JOB_CODE,'J7',SALARY*1.1,'J6',
SALARY*1.15,'J5',SALARY*1.2, SALARY*1.05)"인상된 급여"
FROM EMPLOYEE
...
CASE WHEN SALARY > 5000000  THEN '1등급'
     WHEN SALARY > 3500000  THEN '2등급' -- SALARY >= 5000000 AND SALARY>3500000 둘다맞음
     WHEN SALARY > 2000000  THEN'3등급'  -- SALARY >= 3500000 AND SALARY>2000000
     ELSE '4등급'
     END "급여등급"
FROM EMPLOYEE    
ORDER BY "급여등급";
...
-- 부서 코드가 D5인 사원들의 총 연봉 합

SELECT TO_CHAR(SUM(SALARY*12), 'FM999,999,999') --TO_CHAR는 중간에 ,넣을라고 하는거 필수적인건 아님
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
...
*/
-- 전체 사원의 평균 급여 조회
SELECT TO_CHAR(ROUND(AVG(SALARY)),'FM99,999,999')"평균 급여" -- ROUND 소숫점 제거
FROM EMPLOYEE;

SELECT AVG(BONUS) FROM EMPLOYEE;
SELECT AVG(NVL(BONUS,0)) FROM EMPLOYEE; -- 평균값에 NULL을 제외해서 위랑 결과값이 다름 NVL 무조건 권장
...
FROM EMPLOYEE --FROM EMPLOYEE; 일케 만줘도됨
