...
/*
  ...
  * 테이블 생성 (주요 제일많이씀)
  - 테이블이란? 행(ROW)과 열(COLUMN)로 구성되는 가장 기본적인 데이터베이스 객체 데이터베이스 내에 모든 데이터는 테이블에 저장됨
  ...
  * 자료형
  1. 문자(CHAR / VARCHAR2) - 반드시 크기를 지정해야함
          - CHAR : 최대 2000 BYTE까지 저장가능 
                 고정 길이(아무리 적은 값이 들어와도 처음 할당된 크기 그대로)
                 고정된 글자수의 데이터만이 담길 경우 사용
 자주사용함 - VARCHAR2 : 최대 4000 BYTE까지 저장가능
...
/*
    * 제약조건(CONSTRAINTS)
    ...
    1) 컬럼 레벨 방식 
    CREATE TABLE 테이블명 (
        컬럼명 자료형(크기)[CONSTRAINT 제약조건명] 제약조건, ...  (생략 가능하긴함)
    );
    
    2) 테이블 레벨 방식
    CREATE TABLE 테이블명(
        컬럼명 자료형(크기), ... [CONSTRAINT 제약조건명] 제약조건(컬럼명) 
    );
*/
...
-- 테이블 레벨 방식 : 모든 컬럼들 다 나열 후 마지막에 기술한다.
...
CREATE TABLE MEM_CHECK(
    ...
    MEM_PWD VARCHAR2(20) NOT NULL,
    MEM_NAME VARCHAR2(20) NOT NULL,
    GENDER CHAR(3) CHECK(GENDER IN ('남','여')) NOT NULL,  -- 성별엔 남여만 넣을수 있게함
    PHONE VARCHAR2(13),
    EMAIL VARCHAR2(50) 
);
-- NOT NULL이랑 CHECK 같이 사용
...
CREATE TABLE MEM_PRI2(
    ...
    PHONE VARCHAR2(13),
    EMAIL VARCHAR2(50),
    PRIMARY KEY(MEM_NO, MEM_ID) --> 묶어서 PRIMARY KEY 제약조건 부여 (복합키 라고함) 잘안쓰이긴함 
);
...
-- 추천은 안함  복합키 사용 예시 (어떤 회원이 어떤 상품을 찜하는지에 대한 데이터를 보관하는 테이블)
...
--> 유효한 회원등급번호가 아님에도 불구하고 INSERT되는중 아직 연결 안된것
...
/*
    * FOREIGN KEY(외래키) 제약조건
    ...
    --> 참조할컬럼명 생략시 참조할테이블명에 PRIMARY KEY로 지정된 컬럼으로 매칭함
*/
DROP TABLE MEM;
CREATE TABLE MEM(
    MEM_NO NUMBER PRIMARY KEY,
    ...
    PHONE VARCHAR2(13),
    EMAIL VARCHAR2(50), 
    GRADE_ID NUMBER REFERENCES MEM_GRADE --(GRADE_CODE) --> 컬럼 레벨방식 PRIMARY KEY이기 때문에 생락함
 -- GRADE_ID NUMBERFOREIGN KEY(GRADE_ID) REFERENCES MEM_GRADE(GRADE_CODE) -- > 테이블레벨방식
);
...
DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 30; --> 참조하고있는게 없어서 삭제가됨
...
/*
    ...
    * 삭제옵션 : 부모테이블의 데이터 삭제 시 그 데이트를 사용하고 있는 자식테이블의 값을 어떻게 처리할 건지
    
        3가지 삭제 옵션
        
    - ON DELTE RESTRICTED(기본값)
...
CREATE TABLE TB_BOOK(
    ...
    BK_PRICE NUMBER,
    BK_PUB_NO REFERENCES TB_PUBLISHER ON DELETE CASCADE 
   /* CONSTRAINT TB_BOOK_BK_NO_PK PRIMARY KEY(BK_NO),
    CONSTRAINT TB_BOOK_BK_PUB_NO_PK FOREIGN KEY(BK_PUB_NO) REFERENCES TB_PUBLISHER ON DELETE CASCADE*/
);
...
CREATE TABLE TB_RENT(
    ...
    RENT_BOOK_NO REFERENCES TB_BOOK ON DELETE SET NULL,   
    RENT_DATE DATE DEFAULT SYSDATE
    -- CONSTRAINT TB_RENT_NO_PK PRIMARY KEY(RENT_NO)
    -- CONSTRAINT TB_MEMBER_NO_FK FOREIGN KEY(RENT_NO) REFERENCES TB_MEMBER ON DELETE SET NULL;
    --CONSTRAINT TB_BOOK_NO_FK FOREIGN(RENT_BOOK_NO) REFERENCES TB_BOOK ON DELETE SET NULL;
    );    
...
/*--5.
    ...
    WHERE TM.MEMBER_NO = TR_RENT_MEM_NO
    AND RENT_BOOK_NO = 2;
...
--6.
    ...
    FROM TB_RENT TR 
    JOIN TB_BOOK TB ON (TR.RENT_BOOKNNO = TB.BK_NO)
    JOIN TB_PUBLISHER TP ON (TB.BK_PUB_NO = TP.PUB.NO)
    WHERE RENT_MEM_NO = 1;
