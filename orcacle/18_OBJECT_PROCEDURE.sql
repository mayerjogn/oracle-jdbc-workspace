/*
    ...
    - PL/SQL 문을 저장하여 필요할 때마다 복잡한 구문을 다시 입력할 필요 없이 간단하게 호출해서 실행 결과를 얻는다.
        일회성인 PL/SQL을 저장해서 사용하는거 VIEW가 SELECT문을 저장해서 사용하는거 처럼
        ...
        * 사용
        EXECUTE(또는 EXEC) 프로시저명[(매개값1, 매개값2,...)];(매개값 생략가능)
...
-- 변수가 여러개면 선언한 순서대로 들어감
CREATE PROCEDURE DEL_EMP_ID
(
    P_EMP_ID EMP_COPY.EMP_ID%TYPE --매개 변수를 지정한것
)
IS
BEGIN
    DELETE FROM EMP_COPY
    WHERE EMP_ID = P_EMP_ID;   -- 이쪽 실행부에서 실행
END;
/
ROLLBACK;
...
/*
    ...
    IN은 기본값 IN | OUT 구분하기 위해 IN을 명시한것
*/
-- 사원을 입력받아서 해당하는 사원의 이름, 급여, 보너스를 전달하는 프로시저 생성
-- OUT 리턴값이라 생각
...
-- 바인드 변수의 값을 출력하기 위해서 PRINT 구문을 사용해야함
PRINT VAR_EMP_NAME;
PRINT VAR_SALARY;
PRINT VAR_BONUS;
