/*
    ...
    * PL/SQL 구조
    ...
    - [예외처리부(EXCEPTION SECTION)] : EXCEPTION으로 시작, 예외 발생시 해결하기 위한 구문을 미리 기술해둘 수 있는 부분
*/
-- 출력 기능 활성화
SET SERVEROUTPUT ON;
...
*/
/*
    1_1) 일반 타입 변수 선언 및 초기화  (CONSTANT 넣으면 변수 없으면 상수)
        [표현식]
        변수명[CONSTANT] 자료형 [:=값];
*/
DECLARE
...
-- 레퍼런스 타입의 변수로 EID, ENMAE, JCODE, DTITLE, SAL를 선언하고
-- 각 변수의 자료형은 EMPLOYEE 테이블의 EMP_ID, EMP_NAME, JOB_CODE, SALARY 컬럼과
-- DEPARTMENT 테이블의  DEPT_TITLE 컬럼의 자료형을 참조하고
...
BEGIN
    ...
    INTO EID, ENAME, DTITLE, JCODE, SAL
    FROM EMPLOYEE
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('사번 : ' || EMP.ENM_ID);
    DBMS_OUTPUT.PUT_LINE('이름 : ' || EMP.ENM_NAME);
...
BEGIN SELECT EMP_ID, EMP_NAME, SALARY, NVL(BONUS,0)
...
-- 사번(EID), 이름(ENAME), 부서(DTITLE) 소속에 대해 출력
...
BEGIN SELECT EMP_ID, EMP_NAME, DEPT_TITLE, NATIONAL_CODE
...
-- 입력된 점수에 따라 GRADE(데이터타입 : CHAR(1)) 변수에 저장
-- 90점 이상은 'A', 80점 이상은 'B', 70점 이상은 'C', 60점 이상은 'D', 60점 이하는 'F'
