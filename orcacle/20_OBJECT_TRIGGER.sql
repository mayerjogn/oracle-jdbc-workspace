    ...
    - 테이블이나 뷰가 DML(INSERT, UPDATE, DELETE)문에 의해 변경될 경우
      ...
      * 트리거 종류
      ...
      - SQL문에 의해 영향을 받는 각 행에 따른 분류  
        > STATEMENTE TRIGGER(문장 트리거) : 이벤트가 발생한 SQL문에 대해 딱 한번만 트리거 실행
        
        > ROW TRIGGER(행 트리거) : 해당 SQL문 실행할 때마다 매번 
        트리거 실행(FOR EACH ROW 옵션 기술해야됨) > :OLD - BEFORE UPDATE(수정전 자료), BEFORE DELETE(삭제전 자료)
        > :NEW - AFTER INSERT(추가된 자료), AFTER UPDATE(수정후 자료)
  
    * 트리거 생성
      [표현식]
      CREATE [OR REPLACE] TRIGGER 트리거명
      BEFORE | AFTER | UPDATE | DELETE ON 테이블명 
      [FOR EACH ROW] 행 트리거 작성할때 필요 문장식은 필요 X
      
      DECLARE
        변수선언;
      BEGIN
        실행내용(해당 위에 지정된 이벤트가 발생시 자동으로 실행할 구문)
      EXCEPTION
         예외처리구문;        
      END;
      /
