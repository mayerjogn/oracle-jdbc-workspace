 /*
    ...
    2. 데이터를 테이블 형태로 저장하고 여러 테이블을 조합하여 비즈니스 관계를 도출하는 구조다
    
    3. 데이터의 중복을 최소화할 수 있으며 사용하기 편리하고 데이터의 무결성, 트랜잭션 처리등
 ...
 */
-- EMPLOYEE 테이블에서 직원명(EMP_NAME), 직원의 연봉 조회(=급여 SALARY*12)조회
...
-- 오늘날짜 : SYSDATE 
-- CEIL(NUMBER가 들어감) : 매개값으로 전달되는 수를 올림하는 함수 (소숫점 제거)
...
SELECT EMP_NAME AS 직원명 , SALARY*12 AS 연봉 , (SALARY+(BONUS*SALARY))*12 "총 소득"
FROM EMPLOYEE;
...
/*
    DISTINCT
    - 컬럼에 중복된 값들은 한번씩만 표시하고자 할 때 사용함
*/
-- EMPLOYEE에 직급코드 조회
...
-- 부서코드가 'D1'이 아닌 사원들의 사번, 사원명, 부서코드
...
SELECT EMP_NAME, DEPT_CODE ,SALARY
FROM EMPLOYEE
WHERE DEPT_CODE ='D' or SALARY >= 3000000;

-- 2. 급여가 350만원 이상 600만원 이하를 받는 사원들의 사원명, 급여조회

SELECT EMP_NAME,EMP_ID,SALARY
FROM EMPLOYEE
WHERE SALARY BETWEEN 3500000 AND 60000000;
...
/*
    ...
    - 몇 이상 몇 이하인 범위에 대한 조건을 제시할때 사용되는 연산자이다
...
/*
    ...
    '%' : 0글자 이상을 뜻함
- 비교대상컬럼 LIKE '문자%' => 비교대상컬럼 값이 문자로 "시작"되는걸 조회한다
- 비교대상컬럼 LIKE '%문자' => 비교대상컬럼 값이 문자로 "끝"나는걸 조회한다
- 비교대상컬럼 LIKE '%문자%' => 비교대상컬럼 값에 문자가 "포함"되는걸 조회한다.(키워드 검색시)


    '_' : 1글자를 뜻함
- 비교대상컬럼 LIKE '_문자' => 비교대상컬럼값에 문자앞에 무조건 한글자가 올 경우 조회한다  
- 비교대상컬럼 LIKE '__문자' => 비교대상컬럼값에 문자앞에 무조건 두글자가 올 경우 조회한다  
- 비교대상컬럼 LIKE '_문자_' => 비교대상컬럼값에 문자 앞과 뒤에 무조건 한글자씩 올 경우 조회한다
*/
...
-- 이름 중에 '하'가 포함된 사원명, 주민번호, 전화번호 조회
...
/*
    IN
    - 비교대상컬럼 값이 내가 제시한 목록 중에 일치하는 값이 있는지
    
    [표현법]
    비교대상컬럼 IN ('값1', '값2', ...)
*/

-- 부서 코드가 D6, D8, D5를 제외한 부서원들의 이름, 부서코드, 급여조회
-- 논리 부정 연산자 : NOT 사용 기억해라
...
--5. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 사원들의 사번, 사원명, 급여
-- 보너스 포함연봉 조회(별칭)

SELECT EMP_ID, EMP_NAME,(SALARY+(BONUS*SALARY))*12 AS "총 소득"
FROM EMPLOYEE
...
/*
    ...
    - NULLS FIRST : 정렬하고자하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 앞 배치
                                                         (생략시 DESC일때 기본값)
                                                         
    - NULLS LAST : 정렬하고자하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 뒤 배치
                                                        (생략시 ASC일때 기본값)
*/
...
FROM EMPLOYEE
--WHERE BONUS IS NOT NULL;  null 없앤거
--ORDER BY BONUS; 보너스 오름차순 정렬
ORDER BY BONUS DESC NULLS LAST; -- 보너스 내림차순 정렬 null을 맨뒤로
