...
FROM EMPLOYEE 
WHERE SALARY IN (289000, 3660000, 8000000, 3760000, 3900000, 249000, 255000);
...
-- 사원에 대해 사번, 이름, 부서 코드, 구분(사원/사수) 조회
...
FROM EMPLOYEE E 
JOIN JOB J USING(JOB_CODE)
WHERE J.JOB_NAME = '과장' ); -- ANSI 구문 같이 가능
...
FROM EMPLOYEE E, JOB J
WHERE E.JOB_CODE = J.JOB_CODE
AND J.JOB_NAME = '과장' ); -- 오라클 구문 같이 사용
...
FROM EMPLOYEE E
JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE)
WHERE JOB_NAME LIKE '대리' AND SALARY >
ANY(SELECT SALARY 
FROM EMPLOYEE
...
FROM EMPLOYEE E
JOIN JOB J USING(JOB_CODE)
WHERE JOB_NAME LIKE  '차장' AND SALARY > ALL(SELECT EMP_ID, EMP_NAME,JOB_NAME,SALARY
FROM EMPLOYEE E, JOB J
WHERE E.JOB_CODE = J.JOB_CODE
    WHERE JOB_NAME ='과장');
...
AND JOB_CODE = (SELECT DEPT_CODE FROM EMPLOYEE WHERE EMP_NAME = '하이유');
...
SELECT DEPT_CODE, EMP_NAME,JOB_CODE, MANAGER_ID
FROM EMPLOYEE
WHERE (JOB_CODE, MANAGER_ID)IN (SELECT JOB_CODE);

-- 코드가 D5이면서 J7인 사원 조회

SELECT EMP_NAME, DEPT_CODE, JOB_CODE,MANAGER_ID
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5' AND JOB_CODE = 'J7';

SELECT DEPT_CODE, JOB_CODE, MANAGER_ID
FROM EMPLOYEE
WHERE (JOB_CODE=) IN (SELECT EMP_NAME, DEPT_CODE, JOB_CODE,MANAGER_ID
FROM EMPLOYEE
WHERE DEPT_CODE = '박나라');
...
-- 별칭 사용해야 오류가 안남
SELECT ROWNUM, DEPT_CODE , "평균급여" --AVG(SALARY)
...
--  RANK() OVER() 공동 19위 2명 뒤에 순위는 21위가됨
SELECT RANK()
...
--  DENSEN_RANK() OVER() 공동 19위 2명 뒤에 순위는 20위가됨
SELECT DENSE_RANK()
    OVER(ORDER BY SALARY DESC),
    EMP_NAME,
    SALARY
...
FROM 
(SELECT RANK() OVER(ORDER BY SALARY DESC), EMP_NAME,SALARY FROM EMPLOYEE)
WHERE ROWNUM <= 5;
...
-- 이게 좀더 명시적 이렇게 자주 사용함 걍 이렇게 쓴다 생각해라
