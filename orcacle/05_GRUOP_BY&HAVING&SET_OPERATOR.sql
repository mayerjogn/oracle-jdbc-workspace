...
COUNT(*)"사원의 수",
COUNT(BONUS)"보너스를 받는 사원의 수 ", 
TO_CHAR(SUM(SALARY), 'FM999,999,999')"총 급여",
...
/*
    HAVING
    - 그룹에 대한 조건을 제시 할때 사용되는 구문
    (주로 그룹 함수한 결과를 가지고 비교 수행)
    
    SELECT문 실행순서 (그냥 쓰는 순서만 알아도됨 아래는 실행 순서)
    
    5. SELECT   * | 조회하고자 하는 컬럼명 "별칭" | 계산식 | 함수식
    1. FROM         조회하고자 하는 테이블 명회
...
/*
    ...
    - CUBE(컬럼1, 컬럼2) : 컬럼1을 가지고 중간집계도 내고 , 컬럼2를 가지고 중간집계를 또 낸다
*/
...
SELECT JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY ROLLUP(JOB_CODE); -- 총 합계가 나옴

-- CUBE
SELECT JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY CUBE(JOB_CODE); -- 총 합계가 위에 나옴

-- 컬럼이 하나일 때는 사실상 CUBE, ROLLUP의 차이점이 딱히 없음


-- 부서 코드와 직급 코드가 같은 사원들의 급여 합계 조회 컬럼 둘 이상의 경우
...
-- ROLLUP
-- 부서의 직급별로 중간집계가나오다 마지막에 총 집계가 나옴
...
/*
    ...
    UNION(합집합) : 두 쿼리문을 수행한 결과값을 하나로 합쳐서 추출 (중복되는 행은 제거가 됨)
...
-- 위 쿼리문 대신 WHERE 절에 OR 연산자를 사용해서 처리 가능하니 쓸일이 별로없다
...
-- 위 커리문 대신 WHERE절에 AND 연산자를 사용해서 처리 가능
SELECT DEPT_CODE, EMP_NAME, EMP_ID, SALARY
FROM EMPLOYEE
