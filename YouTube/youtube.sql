...
-- 테스트와 관련된 쿼리문들!
-- 회원가입(register)
INSERT INTO MEMBER(ID, MEMBER_PASSWORD, MEMBER_NICKNAME)VALUES('user1','1234','꼬랑');-- 나머지 4개는 필수값이 아니라 안넣었음
COMMIT;
SELECT * FROM MEMBER;

-- 로그인(login)
SELECT * FROM MEMBER WHERE ID = 'user2' AND MEMBER_PASSWORD = '1111';

-- 채널 추가(addChannel)
INSERT INTO CHANNEL(CHANNEL_CODE, CHANNEL_NAME, ID) VALUES(SEQ_CHANNEL.NEXTVAL, 'Blue Rain', 'user2');
...
-- 내 채널 보기(myChannel)
SELECT CHANNEL_NAME, MEMBER_NICKNAME FROM CHANNEL JOIN MEMBER USING(ID) WHERE ID = 'user2';
