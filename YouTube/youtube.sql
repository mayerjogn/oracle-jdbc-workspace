...
-- 테스트와 관련된 쿼리문들!
-- 회원가입(register)
INSERT INTO MEMBER(MEMBER_ID, MEMBER_PASSWORD, MEMBER_NICKNAME)VALUES('user1','1234','꼬랑');-- 나머지 4개는 필수값이 아니라 안넣었음
COMMIT;
SELECT * FROM MEMBER;

-- 로그인(login)
SELECT * FROM MEMBER WHERE MEMBER_ID = 'user2' AND MEMBER_PASSWORD = '1111';

-- 채널 추가(addChannel)
INSERT INTO CHANNEL(CHANNEL_CODE, CHANNEL_NAME, MEMBER_ID) VALUES(SEQ_CHANNEL.NEXTVAL, 'Blue Rain', 'user2');
...
-- 내 채널 보기(myChannel)
SELECT CHANNEL_NAME, MEMBER_NICKNAME FROM CHANNEL JOIN MEMBER USING(MEMBER_ID) WHERE MEMBER_ID = 'user2';
...
DELETE FROM CHANNEL WHERE CHANNEL_CODE=4;

-- 비디오 추가
 INSERT INTO VIDEO(VIDEO_CODE, VIDEO_TITLE, VIDEO_URL, VIDEO_PHOTO, CATEGORY_CODE, CHANNEL_CODE, MEMBER_ID)
 VALUES (SEQ_VIDEO.NEXTVAL, '250만원 받던 직장 그만두고 귀어해서 하루 500만원 버는 39살', 'URL01', 'PHOTO01',1, 3, 'user2');
